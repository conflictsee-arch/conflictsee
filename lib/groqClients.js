...
// Groq compound can hang; race every call against a timeout so one
// slow article never stalls the whole ingestion run.
const GROQ_CALL_TIMEOUT_MS = 45000

function withTimeout(promise, ms = GROQ_CALL_TIMEOUT_MS) {
  let timer
  const timeout = new Promise((_, reject) => {
    timer = setTimeout(() => reject(new Error(`Groq call timed out after ${ms}ms`)), ms)
  })
...
// Run async tasks with bounded concurrency (default 3 at a time).
// Used by ingestion routes to call Groq in parallel instead of serially.
