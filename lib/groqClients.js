...
export const SECTION_MODELS = {
  ...
  economics: 'llama-3.3-70b-versatile',
  world_affairs: 'llama-3.3-70b-versatile',
  rumors: 'llama-3.3-70b-versatile',
  backfill: 'llama-3.3-70b-versatile',
}
...
export async function groqWithFallback(prompt, maxTokens = 2000, temperature = 0.3) {
  ...
  for (const key of ALL_KEYS) {
    try {
      return await singleCall(newClient(key), prompt, maxTokens, temperature)
    } catch (err) {
      lastErr = err
      if (err.status === 429) {
        await new Promise((r) => setTimeout(r, 1500))
