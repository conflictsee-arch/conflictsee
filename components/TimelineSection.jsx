...
// Pure JS time conversion (no API), based on UTC + offsetMinutes.
function convertTime(isoDateStr, offsetMinutes) {
  if (!isoDateStr) return '—'
  const utcMs = new Date(isoDateStr).getTime()
...
function getClockEmoji(dateStr, offsetMinutes, mounted) {
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          if (!newRow || newRow.verified !== true) return
          setEvents(prev => {
    ...
    <section style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          >
            {fetching ? '⏳' : '↻'} {fetching ? 'Fetching...' : 'Fetch Latest News'}
          </button>
          {fetchMsg && (
            <span style={{
              fontSize: '12px',
              ...
              <div key={day}>
                ...
                <div className="day-events-container">
                  {sortedDayEvents.map(event => {
                    ...
                    const displayTime = convertTime(event?.published_at, activeTimezone.offsetMinutes)
                      ...
                      >
                        ...
                        <div style={{
                          ...
                          <span style={{
                            ...
                            {getClockEmoji(event?.published_at, activeTimezone.offsetMinutes, mounted)}
                          </span>
                          <span style={{
